...
export async function listAccessibleLibraryItems({
  ...
  const items = snapshot.docs.map((doc: QueryDocumentSnapshot) => ({ id: doc.id, ...doc.data() } as LibraryItem));
  const tokens = tokenizeQuestion(question);

  return items
    .map((item: LibraryItem) => ({ item, score: scoreItem(tokens, item) }))
    .filter(({ score }: { score: number }) => score > 0 || tokens.length === 0)
    .sort((left: { score: number }, right: { score: number }) => right.score - left.score)
    .slice(0, maxItems)
    .map(({ item }: { item: LibraryItem }) => item);
}
...
export async function listLibraryItemsByOwner(ownerUid: string): Promise<LibraryItem[]> {
  ...
  return snapshot.docs.map((doc: QueryDocumentSnapshot) => ({ id: doc.id, ...doc.data() } as LibraryItem));
}
